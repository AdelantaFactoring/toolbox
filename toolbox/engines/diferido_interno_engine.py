"""
⚙️ Diferido Interno Engine V2
LÓGICA COPIADA EXACTA DE DiferidoInternoCalcular V1
"""
...
try:
    ...
    raise ImportError(
        "DiferidoInternoEngine V2 requiere BaseEngine de imports relativos"
    )

locale.setlocale(locale.LC_TIME, "es_ES.UTF-8")


class DiferidoInternoEngine(BaseEngine):
    """Motor que contiene TODA la lógica de DiferidoInternoCalcular V1"""
    ...
    def calcular_diferido_interno(self, df: pd.DataFrame, hasta: str) -> pd.DataFrame:
    ...
    def obtener_resumen(self, df: pd.DataFrame) -> pd.DataFrame:
        """COPIADO EXACTO DE V1"""
        calculos_df = df  # En V2 recibimos el df como parámetro
        resumen_df = pd.DataFrame(columns=["Mes", "PEN", "USD"])
        date_range = pd.date_range(
            df[self.FECHA_OPERACION].min(),
            df[self.FECHA_PAGO_CONFIRMADO].max(),
            freq="ME",
        )
